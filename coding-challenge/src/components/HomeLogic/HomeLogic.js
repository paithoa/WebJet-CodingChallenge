...
import styles from "./HomeLogic.module.css";
...
class HomeLogic extends Component {
    ...
    listOfHotels: [
      {
        ...
        rating: 4.5,
        roomType: "Studio Flat Apartment",
        price: 282,
        imgSrc:
  ...
  componentDidMount() {
    ...
    const sortedHotels = []
      .concat(this.state.listOfHotels)
      .sort((hotelA, hotelB) => (hotelA.price > hotelB.price ? 1 : -1));

    this.setState({ listOfHotels: sortedHotels });
  }
  ...
  filterName = () => {
      let filteredHotelsByName = [];
          //Filter listOfHotels by hotel name
    if (this.state.isFilteredByName) {
        if (this.state.nameFilter !== "") {
          filteredHotelsByName = this.state.listOfHotels.filter(
            (hotel) => hotel.hotelName === this.state.nameFilter
          );
        }
        //If not filtered by name, simply assign all the listOfHotels to filteredHotelsByName
      } else {
        filteredHotelsByName = [...this.state.listOfHotels];
      }
      return filteredHotelsByName
  }

  render() {
    let filteredHotels = [];
    let filteredHotelsByName = this.filterName();
    let CardHotels = null;
    //Filter listOfHotels by rating
    if (this.state.isFilteredByRating) {
      ...
      CardHotels = filteredHotels?.map((hotel, i) => {
        return <CardHotel key={i} details={hotel} />;
      });
    } else {
      //If not filtered by rating, use filteredHotelsByName to create the JSX elements
      CardHotels = filteredHotelsByName.map((hotel, i) => {
        return <CardHotel key={i} details={hotel} />;
      });
    }

    if (CardHotels.length === 0) {
      CardHotels = (
    ...
    return (
      <div className={styles.Search}>
        <h2 className={styles.Heading}>
          {this.state.listOfHotels.length} Hotels Available in Melbourne
        </h2>
        <div className={styles.MainContent}>
          ...
          />
          <div class={styles.CardHotels}>{CardHotels}</div>
          <Advertisement />
        </div>
      </div>
    );
